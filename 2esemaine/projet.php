...
<h4>Accès Rapide</h4>
<ul>
<li><a href="index.php" title="Cours Webmasters - Annuaire Android" target="">Annuaire Android</a></li>
...
<BodyCategoriePresentationSiteWeb class="BodySiteWebCategorie">Une recherche des projets ?</BodyCategoriePresentationSiteWeb>
...
*/ include "recherche.php";
            ...
            $mysqli = mysqli_connect('localhost','retro','test','webmasters');
            //$connexion = mysqli_connect($mysqli);
            

if(isset($_POST['Recherche'])) {
                $id=$_GET['id'];
            $seek = AddSlashes(htmlspecialchars($_POST['Recherche']));
    if($mysqli -> connect_error) {
              echo "Failed to connect to MySQL: " . $mysqli -> connect_error;
              exit();
    }
    if ($result = $mysqli->prepare("SELECT * FROM Annuaire WHERE TitreProjet LIKE '%$seek%'")){
        $result->execute();
        ...
        if(!empty($seek)){
            if($num>0){
                ...
                '$seek' !</span><br>";
                        ...
                        $result = $mysqli->prepare("SELECT * FROM Annuaire");
                        $result->execute();
            }
            while($aff=mysqli_fetch_array($result)){
                if(isset($id) && isset($aff['id'])){
                            $id-> $aff['id'];
                }  
                        echo"
                        <tr class='projet'><td>
                        <span class='KlozaSeek_sites_titre'>
                                           <h3> Titre du projet :</h3>
                                            '".$aff['TitreProjet']."'
                                            </a><br></td>";
                    
                        echo"<td><span class='KlozaSeek_sites_des'>
                                            <h3>Date de mise en ligne du projet :</h3>
                                            '".$aff['DateCreationProjet']."'
                                            </span><br></td>";
                        ...
                        echo"<td><span class='KlozaSeek_sites_infos'>
                                            <h3>Extrait de la description du projet ( SCRIPT PHP ANNUAIRE TELEPHONIQUE TYPE PAGE JAUNNE ) : </h3>
                                            '".$aff['DescriptionProjet']."'
                                            </span><br><br></td></tr>";
                        echo "<button class='open-button' onclick='openForm($userinfo)'>Ajouter un partenaire</button><br>";
                        include 'insert-membre.php';
            }
                    

        }
        else{
            $result = $mysqli->query("SELECT * FROM Annuaire");
